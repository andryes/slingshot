...
class Ai1wmke_Pro_Settings_Controller {

	public static function index() {
		$model = new Ai1wmke_Pro_Settings;

		Ai1wm_Template::render(
			'settings/index/index-pro',
			array(
	...
	public static function settings( $params = array() ) {
		...
		// Settings update
		if ( isset( $params['ai1wmke_pro_update'] ) ) {
			$model = new Ai1wmke_Pro_Settings;

			// Set number of backups
			if ( ! empty( $params['ai1wmke_pro_backups'] ) ) {
				$model->set_backups( (int) $params['ai1wmke_pro_backups'] );
			} else {
				$model->set_backups( 0 );
			}

			// Set size of backups
			if ( ! empty( $params['ai1wmke_pro_total'] ) && ! empty( $params['ai1wmke_pro_total_unit'] ) ) {
				$model->set_total( (int) $params['ai1wmke_pro_total'] . trim( $params['ai1wmke_pro_total_unit'] ) );
			} else {
				$model->set_total( 0 );
			}

			// Set number of days
			if ( ! empty( $params['ai1wmke_pro_days'] ) ) {
				$model->set_days( (int) $params['ai1wmke_pro_days'] );
			} else {
				$model->set_days( 0 );
			}
		...
		wp_redirect( network_admin_url( 'admin.php?page=ai1wmke_pro_settings' ) );
		exit;
	}
			...
			if ( $item->isDir() && ! $item->isLink() && $item->isReadable() ) {
				try {
					$files[] = array(
						...
						'writable' => $item->isWritable() && realpath( $name ) !== realpath( ABSPATH ),
