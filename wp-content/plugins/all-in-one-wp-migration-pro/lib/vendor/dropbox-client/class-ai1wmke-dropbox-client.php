...
class Ai1wmke_Dropbox_Client {
	...
	public function __construct( $token, $ssl = true ) {
		if ( get_option( 'ai1wmke_dropbox_offline', false ) ) {
			$this->refresh_token = $token;
		} else {
			static::$access_token = $token;
		}

		$this->full_access = (bool) get_option( 'ai1wmke_dropbox_full_access' );
		$this->ssl         = $ssl;
	}
	...
	public function upload_file( $file_data, $file_path ) {
		...
		$api->set_header(
			...
			json_encode(
				array(
					'path'       => $this->sanitize_path( $file_path ),
					'mode'       => 'overwrite',
					'autorename' => false,
					'mute'       => false,
				)
	...
	public function upload_next_file_chunk( $file_chunk_data, $session_id, $file_range_start = 0 ) {
		...
		try {
			...
			$this->set_retry_after( $api->get_response_header( 'retry-after' ) );
			throw $e;
		} catch ( Ai1wmke_Incorrect_Offset_Exception $e ) {
	...
	public function upload_file_chunk_commit( $file_chunk_data, $file_path, $session_id, $file_range_start = 0 ) {
		...
		try {
			...
			$this->set_retry_after( $api->get_response_header( 'retry-after' ) );
			throw $e;
		} catch ( Ai1wmke_Lookup_Failed_Exception $e ) {
			$this->set_correct_offset( $api->get_correct_offset() );
			throw $e;
		}
	...
	public function get_file( $file_stream, $file_path, $file_range_start = 0, $file_range_end = 0 ) {
		...
		$api->set_header( 'Dropbox-API-Arg', json_encode( array( 'path' => $this->sanitize_path( $file_path ) ) ) );
		...
		if ( fwrite( $file_stream, $file_chunk_data ) === false ) {
			throw new Ai1wmke_Error_Exception( __( 'Unable to save the file from Dropbox', AI1WMKE_PLUGIN_NAME ) );
		}
	...
	public function create_folder( $folder_path ) {
		...
		$api->set_option(
			...
			json_encode(
				array(
					'path'       => $this->sanitize_path( $folder_path ),
					'autorename' => false,
				)
			)
		);
	...
	public function create_shared_link( $folder_path ) {
		...
		$api->set_header( 'Content-Type', 'application/json; charset=utf-8' );
		$api->set_option( CURLOPT_POST, true );
		$api->set_option( CURLOPT_POSTFIELDS, json_encode( array( 'path' => $this->sanitize_path( $folder_path ) ) ) );

		try {
			$response = $api->make_request( true );
		} catch ( Ai1wmke_Shared_Link_Already_Exists_Exception $e ) {
	...
	public function get_access_token() {
		...
		if ( time() < get_option( 'ai1wmke_dropbox_access_token_expires_in', false ) && ( static::$access_token = get_option( 'ai1wmke_dropbox_access_token', false ) ) ) {
			return static::$access_token;
		}
		...
		$api->set_ssl( $this->ssl );
		if ( $this->full_access ) {
			$api->set_base_url( AI1WMKE_DROPBOX_REFRESH_FULL_URL );
		} else {
			$api->set_base_url( AI1WMKE_DROPBOX_REFRESH_URL );
		}
		$api->set_option( CURLOPT_POST, true );
		$api->set_option(
			CURLOPT_POSTFIELDS,
		...
		if ( isset( $response['access_token'] ) ) {
			...
			update_option( 'ai1wmke_dropbox_access_token', $response['access_token'] );
		}

		if ( isset( $response['expires_in'] ) ) {
			update_option( 'ai1wmke_dropbox_access_token_expires_in', time() + ( $response['expires_in'] - 10 * 60 ) );
		}

		if ( isset( $response['refresh_token'] ) && $response['refresh_token'] !== $this->refresh_token ) {
			$this->refresh_token = $response['refresh_token'];
			update_option( 'ai1wmke_dropbox_token', $response['refresh_token'] );
		}
