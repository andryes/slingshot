...
class Ai1wmke_WebDAV_Client {
	...
	protected function make_request( $path, $options = array() ) {
		...
		if ( $this->ssl ) {
			...
			curl_setopt( $this->handler, CURLOPT_CAINFO, dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'certs' . DIRECTORY_SEPARATOR . 'cacert.pem' );
			curl_setopt( $this->handler, CURLOPT_CAPATH, dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'certs' );
		}
