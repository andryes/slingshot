...
class Ai1wmke_Backup_CLI_Command extends Ai1wmke_Backup_CLI_Base {

	public function __construct( $params = array() ) {
		parent::__construct( array_merge( $params, array( 'file' => 1 ) ) );
	}
	...
	public function backup( $args = array(), $assoc_args = array() ) {
		...
		WP_CLI::log( sprintf( __( 'Backup location: %s', AI1WMKE_PLUGIN_NAME ), ai1wm_backup_path( $params ) ) );
	}
		...
		foreach ( $model->get_files() as $backup ) {
			$backups->addRow(
				array(
					...
					'date' => sprintf( __( '%s ago', AI1WMKE_PLUGIN_NAME ), human_time_diff( $backup['mtime'] ) ),
	...
	public function browse_backup( $args = array(), $assoc_args = array() ) {
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'A backup name must be provided in order to proceed with the list backup files process.', AI1WMKE_PLUGIN_NAME ),
					__( 'Example: wp ai1wm backup_files migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( ! is_file( ai1wm_backup_path( array( 'archive' => $args[0] ) ) ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'The backup file could not be located in wp-content/ai1wm-backups folder.', AI1WMKE_PLUGIN_NAME ),
					__( 'To list available backups use: wp ai1wm list-backups', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		try {
			...
			$table->setHeaders(
				array(
					'name' => __( 'File name', AI1WMKE_PLUGIN_NAME ),
					'date' => __( 'Date created', AI1WMKE_PLUGIN_NAME ),
					'size' => __( 'Size', AI1WMKE_PLUGIN_NAME ),
				)
			);
			...
			foreach ( $files as $file ) {
				$table->addRow(
					array(
						'name' => $file['filename'],
						'date' => sprintf( __( '%s ago', AI1WMKE_PLUGIN_NAME ), human_time_diff( $file['mtime'] ) ),
	...
	public function extract_backup( $args = array(), $assoc_args = array() ) {
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'A backup name must be provided in order to proceed with the extract process.', AI1WMKE_PLUGIN_NAME ),
					__( 'Example: wp ai1wm backup-extract migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( ! isset( $assoc_args['extract-path'] ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'A backup extract path must be provided and writable in order to proceed with the extract process.', AI1WMKE_PLUGIN_NAME ),
					__( 'Example: wp ai1wm backup-extract migration-wp-20170913-095743-931.wpress --extract-path=/tmp', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( ! is_writable( $params['extract_path'] ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'The extract folder is not writable. Check that directory exists and writable.', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		try {
			...
			WP_CLI::error( sprintf( __( 'Unable to extract files: %s', AI1WMKE_PLUGIN_NAME ), $e->getMessage() ) );
			exit;
		}
	}
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'Example: wp ai1wm restore migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	public function reset( $args = array(), $assoc_args = array() ) {
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'A reset type must be provided in order to proceed with the reset process.', AI1WMKE_PLUGIN_NAME ),
					__( 'Example: wp ai1wm reset database', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}

		$reset_types = array( 'plugins', 'themes', 'media', 'database', 'full' );
		if ( ! in_array( $args[0], $reset_types ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'Unsupported reset type.', AI1WMKE_PLUGIN_NAME ),
					sprintf( __( 'Supported types are: %s', AI1WMKE_PLUGIN_NAME ), implode( ', ', $reset_types ) ),
				)
			);
			exit;
		}
		...
		if ( ! current_user_can( 'export' ) ) {
			WP_CLI::error( __( 'Insufficient permissions to run reset.', AI1WMKE_PLUGIN_NAME ) );
			exit;
		}
		...
		$params['ai1wm_reset_password'] = readline( sprintf( __( 'To continue, enter your current user [%s] password: ', AI1WMKE_PLUGIN_NAME ), $user->user_login ) );
		...
		try {
			Ai1wmke_Reset_Controller::reset( $params );
		} catch ( Exception $e ) {
			WP_CLI::error( __( sprintf( 'Unable to reset: %s', $e->getMessage() ), AI1WMKE_PLUGIN_NAME ) );
			exit;
		}

		WP_CLI::success( __( 'Reset completed.', AI1WMKE_PLUGIN_NAME ) );
	}
	...
	public function check_for_updates( $args = array(), $assoc_args = array() ) {
		try {
			...
			WP_CLI::error( __( sprintf( 'Unable to check for updates: %s', $e->getMessage() ), AI1WMKE_PLUGIN_NAME ) );
			exit;
		}

		WP_CLI::success( __( 'Check for updates completed.', AI1WMKE_PLUGIN_NAME ) );
	}
}
