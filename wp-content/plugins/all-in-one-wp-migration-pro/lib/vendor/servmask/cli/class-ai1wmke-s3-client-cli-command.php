 ...
 * ## EXAMPLES
 *
 *     # Backup site to S3 Client.
 *     $ wp ai1wm s3-client backup
 *     Success: Backup completed.
 *
 *     # List existing S3 Client backups.
 *     $ wp ai1wm s3-client list-backups
 *     +--------------------------------------+--------------+----------+
 ...
 *     +--------------------------------------+--------------+----------+
 *
 *     # Restore website from S3 Client backups.
 *     $ wp ai1wm s3-client restore wordpress-20181123-071143-621.wpress --yes
 *     Success: Restore completed.
 */
class Ai1wmke_S3_Client_CLI_Command extends Ai1wmke_Backup_CLI_Base {

	public function __construct( $params = array() ) {
		parent::__construct( array_merge( $params, array( 's3-client' => 1 ) ) );

		// Check connection details
		if ( ! get_option( 'ai1wmke_s3_client_access_key', ai1wmke_aws_access_key() ) || ! get_option( 'ai1wmke_s3_client_secret_key', ai1wmke_aws_secret_key() ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'In order to use S3 Client you need to configure it first.', AI1WMKE_PLUGIN_NAME ),
					__( 'Please navigate to WP Admin > All-in-One WP Migration > S3 Client Settings and configure your S3 Client connection details.', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	/**
	 * Creates a new backup and uploads to S3-compatible storage.
	 *
	 * ## OPTIONS
	 *
	 ...
	 * ## EXAMPLES
	 *
	 * $ wp ai1wm s3-client backup --replace "wp" "WordPress"
	 * Backup in progress...
	 * Uploading wordpress-20190509-152915-654.wpress (33 MB) [60% complete]
	 * Uploading wordpress-20190509-152915-654.wpress (33 MB) [100% complete]
	 * Success: Backup completed.
	 * Backup file: wordpress-20190509-152915-654.wpress
	 * Backup location: [region: us-east-1] [bucket: ai1wm-servmask-com] [path: backups/daily/ai1wm/wordpress-20190509-152915-654.wpress]
	 *
	 * @subcommand backup
	 */
	...
	/**
	 * Get a list of S3 Client backup files.
	 *
	 * ## OPTIONS
	 *
	 * [--folder-path=<path>]
	 * : List backups in a specific S3 Client subfolder
	 *
	 * ## EXAMPLES
	 *
	 * $ wp ai1wm s3-client list-backups
	 ...
	 * +------------------------------------------------+--------------+-----------+
	 *
	 * $ wp ai1wm s3-client list-backups --folder-path=/backups/daily
	 * +------------------------------------------------+--------------+-----------+
	 * | Backup name                                    | Date created | Size      |
	 * +------------------------------------------------+--------------+-----------+
	 * | migration-wp-20170908-152313-435.wpress        | 4 days ago   | 536.77 MB |
	...
	/**
	 * Restores a backup from S3 Client.
	 *
	 * ## OPTIONS
	 *
	 * <file>
	 ...
	 *
	 * [--folder-path=<path>]
	 * : Download a backup from a specific S3 Client subfolder
	 *
	 * [--yes]
	 ...
	 * ## EXAMPLES
	 *
	 * $ wp ai1wm s3-client restore migration-wp-20170913-095743-931.wpress
	 * Restore in progress...
	 * Restore completed.
	 *
	 * $ wp ai1wm s3-client restore migration-wp-20170913-095743-931.wpress --folder-path=/backups/daily
	 * @subcommand restore
	 */
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'Example: wp ai1wm s3-client restore migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( is_null( $file ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'To list available backups use: wp ai1wm s3-client list-backups', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	protected function list_items( $folder_path ) {
		...
		$bucket_name = get_option( 'ai1wmke_s3_client_bucket_name', ai1wm_archive_bucket() );
		$region_name = $s3->get_bucket_location( $bucket_name, get_option( 'ai1wmke_s3_client_region_name', ai1wmke_aws_region_name( AI1WMKE_S3_CLIENT_REGION_NAME ) ) );
	...
	protected function get_folder_path( $assoc_args ) {
		...
		$folder_path = get_option( 'ai1wmke_s3_client_folder_name', '' );
