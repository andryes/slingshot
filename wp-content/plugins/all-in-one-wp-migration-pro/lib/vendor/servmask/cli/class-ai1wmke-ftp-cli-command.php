 ...
 *     +--------------------------------------+--------------+----------+
 *
 *     # Restore website from FTP backups.
 *     $ wp ai1wm ftp restore wordpress-20181123-071143-621.wpress --yes
 *     Success: Restore completed.
 */
class Ai1wmke_FTP_CLI_Command extends Ai1wmke_Backup_CLI_Base {

	public function __construct( $params = array() ) {
		parent::__construct( array_merge( $params, array( 'ftp' => 1 ) ) );

		// Check connection details
		if ( ! get_option( 'ai1wmke_ftp_hostname', false ) || ! get_option( 'ai1wmke_ftp_username', false ) || ! get_option( 'ai1wmke_ftp_password', false ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'In order to use FTP you need to configure it first.', AI1WMKE_PLUGIN_NAME ),
					__( 'Please navigate to WP Admin > All-in-One WP Migration > FTP Settings and configure your FTP connection details.', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'Example: wp ai1wm ftp restore migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( is_null( $file ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'To list available backups use: wp ai1wm ftp list-backups', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	protected function get_backup_uri( $params ) {
		// Set file path
		$folder_path = get_option( 'ai1wmke_ftp_directory', '' );
