 ...
 * ## EXAMPLES
 *
 *     # Backup site to Google Drive.
 *     $ wp ai1wm gdrive backup
 *     Success: Backup completed.
 *
 *     # List existing Google Drive backups.
 ...
 *     +--------------------------------------+--------------+----------+
 *
 *     # Restore website from Google Drive backups.
 *     $ wp ai1wm gdrive restore wordpress-20181123-071143-621.wpress --yes
 *     Success: Restore completed.
 */
class Ai1wmke_GDrive_CLI_Command extends Ai1wmke_Backup_CLI_Base {

	public function __construct( $params = array() ) {
		parent::__construct( array_merge( $params, array( 'gdrive' => 1 ) ) );

		// Check connection details
		if ( ! get_option( 'ai1wmke_gdrive_token', false ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'In order to use Google Drive you need to configure it first.', AI1WMKE_PLUGIN_NAME ),
					__( 'Please navigate to WP Admin > All-in-One WP Migration > Google Drive Settings and Link your Google Drive account.', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	public function list_backups( $args = array(), $assoc_args = array() ) {
		...
		if ( ( $folder_id = $this->get_folder_id( $assoc_args, $team_drive_id ) ) ) {
			...
			do {
				...
				if ( isset( $data['token'] ) ) {
					WP_CLI::log( __( 'Press enter key to continue...', AI1WMKE_PLUGIN_NAME ) );
					fgetc( STDIN );
				}
	...
	public function restore( $args = array(), $assoc_args = array() ) {
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'Example: wp ai1wm gdrive restore migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( ! isset( $file ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'The backup file could not be located in this folder.', AI1WMKE_PLUGIN_NAME ),
					__( 'To list available backups use: wp ai1wm gdrive list-backups', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	protected function list_items( $folder_id, $team_drive_id = null, $next_page_token = null ) {
		...
		try {
			$items = $gdrive->list_folder_by_id( $folder_id, $team_drive_id, $next_page_token, "fileExtension = 'wpress'", array( 'orderBy' => 'folder,createdDate desc' ) );
		} catch ( Exception $e ) {
	...
	protected function get_folder_id( $assoc_args, $team_drive_id = null ) {
		...
		return get_option( 'ai1wmke_gdrive_folder_id', null );
	}
