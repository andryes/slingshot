 ...
 *     +--------------------------------------+--------------+----------+
 *
 *     # Restore website from Dropbox backups.
 *     $ wp ai1wm dropbox restore wordpress-20181123-071143-621.wpress --yes
 *     Success: Restore completed.
 */
class Ai1wmke_Dropbox_CLI_Command extends Ai1wmke_Backup_CLI_Base {

	public function __construct( $params = array() ) {
		parent::__construct( array_merge( $params, array( 'dropbox' => 1 ) ) );

		// Check connection details
		if ( ! get_option( 'ai1wmke_dropbox_token', false ) ) {
			WP_CLI::error_multi_line(
				array(
					__( 'In order to use Dropbox you need to configure it first.', AI1WMKE_PLUGIN_NAME ),
					__( 'Please navigate to WP Admin > All-in-One WP Migration > Dropbox Settings and Link your Dropbox account.', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	public function backup( $args = array(), $assoc_args = array() ) {
		...
		if ( $shared_link = get_option( 'ai1wmke_dropbox_folder_shared_link', false ) ) {
			WP_CLI::log( sprintf( __( 'Backup location: %s', AI1WMKE_PLUGIN_NAME ), $shared_link ) );
		}
	}
		...
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'Example: wp ai1wm dropbox restore migration-wp-20170913-095743-931.wpress', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
		...
		if ( is_null( $file ) ) {
			WP_CLI::error_multi_line(
				array(
					...
					__( 'To list available backups use: wp ai1wm dropbox list-backups', AI1WMKE_PLUGIN_NAME ),
				)
			);
			exit;
		}
	...
	protected function list_items( $folder_path ) {
		...
		try {
			$result = $dropbox->list_folder( $folder_path, array( 'folder', 'file' => '/\.wpress$/' ) );
		} catch ( Exception $e ) {
	...
	protected function get_folder_path( $assoc_args ) {
		...
		return get_option( 'ai1wmke_dropbox_folder_path', null );
	}
}
