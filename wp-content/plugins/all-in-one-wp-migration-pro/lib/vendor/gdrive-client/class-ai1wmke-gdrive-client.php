...
class Ai1wmke_GDrive_Client {
	...
	public function upload_file( $file_data, $file_name, $parent_id = 'root', $team_drive_id = null ) {
		if ( get_option( 'ai1wmke_gdrive_app_folder', false ) ) {
			if ( $parent_id === 'root' ) {
				$parent_id = 'appDataFolder';
			}
		} elseif ( $team_drive_id !== AI1WMKE_GDRIVE_TEAM_DRIVE_ID ) {
			if ( $parent_id === 'root' ) {
	...
	public function get_file( $file_stream, $file_range_start = 0, $file_range_end = 0 ) {
		...
		if ( fwrite( $file_stream, $file_chunk_data ) === false ) {
			throw new Ai1wmke_Error_Exception( __( 'Unable to save the file from Google Drive', AI1WMKE_PLUGIN_NAME ) );
		}
	...
	public function list_team_drives() {
		...
		$data = array(
			'items' => array(
				array(
					'id'   => AI1WMKE_GDRIVE_TEAM_DRIVE_ID,
					'name' => __( 'My Drive', AI1WMKE_PLUGIN_NAME ),
					'type' => 'drive',
				),
			),
		);
	...
	public function get_access_token() {
		...
		if ( time() < get_option( 'ai1wmke_gdrive_access_token_expires_in', false ) && ( static::$access_token = get_option( 'ai1wmke_gdrive_access_token', false ) ) ) {
			return static::$access_token;
		}
		...
		$api->set_header( 'Content-Type', 'application/json' );
		$api->set_ssl( $this->ssl );
		$api->set_base_url( AI1WMKE_GDRIVE_REFRESH_URL );
		$api->set_option( CURLOPT_POST, true );
		$api->set_option(
			CURLOPT_POSTFIELDS,
			json_encode(
		...
		if ( isset( $response['access_token'] ) ) {
			...
			update_option( 'ai1wmke_gdrive_access_token', $response['access_token'] );
		}

		if ( isset( $response['expires_in'] ) ) {
			update_option( 'ai1wmke_gdrive_access_token_expires_in', time() + ( $response['expires_in'] - 10 * 60 ) );
		}

		if ( isset( $response['refresh_token'] ) && $response['refresh_token'] !== $this->refresh_token ) {
			$this->refresh_token = $response['refresh_token'];
			update_option( 'ai1wmke_gdrive_token', $response['refresh_token'] );
		}
