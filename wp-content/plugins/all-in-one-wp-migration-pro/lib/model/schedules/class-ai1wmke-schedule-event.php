...
class Ai1wmke_Schedule_Event {
	...
	const CRON_HOOK = 'ai1wmke_schedule_event';
	...
	public function __construct( $data ) {
		if ( ! is_array( $data ) ) {
			throw new Ai1wmke_Schedules_Exception( __( 'Event data must be array', AI1WMKE_PLUGIN_NAME ) );
		}
	...
	public function period() {
		if ( ! $this->repeating ) {
			return __( 'Do not repeat', AI1WMKE_PLUGIN_NAME );
		}

		switch ( $this->schedule['interval'] ) {
			case self::INTERVAL_HOURLY:
				return $this->incremental ? __( 'Continuous', AI1WMKE_PLUGIN_NAME ) : __( 'Per hour', AI1WMKE_PLUGIN_NAME );

			case self::INTERVAL_DAILY:
				return $this->incremental ? __( 'Once per day', AI1WMKE_PLUGIN_NAME ) : __( 'Per day', AI1WMKE_PLUGIN_NAME );

			case self::INTERVAL_WEEKLY:
				return __( 'Per week', AI1WMKE_PLUGIN_NAME );

			case self::INTERVAL_MONTHLY:
				return __( 'Per month', AI1WMKE_PLUGIN_NAME );

			default:
				return __( 'Unknown period', AI1WMKE_PLUGIN_NAME );
		}
	}
	...
	protected function recurrence() {
		switch ( $this->schedule['interval'] ) {
			...
			default:
				throw new Ai1wmke_Schedules_Exception( __( 'Undefined schedule interval!', AI1WMKE_PLUGIN_NAME ) );
		}
	}
	...
	public function mark_success( $params = null ) {
		...
		$log = new Ai1wmke_Schedule_Event_Log( $this->event_id );
	...
	protected function notify_success( $params ) {
		...
		$notification = sprintf( __( '<p>Your site %s was successfully exported.</p>', AI1WMKE_PLUGIN_NAME ), site_url() ) .
		...
		if ( $file_size ) {
			$notification .= sprintf( __( '<p>Size: %s</p>', AI1WMKE_PLUGIN_NAME ), $file_size );
		}

		Ai1wm_Notification::ok(
			sprintf( __( 'Scheduled event has completed (%s)', AI1WMKE_PLUGIN_NAME ), parse_url( site_url(), PHP_URL_HOST ) . parse_url( site_url(), PHP_URL_PATH ) ),
			$notification
		);
	}
	...
	protected function notify_failed( $error ) {
		...
		$notification = sprintf( __( '<p>Your site %s export failed.</p>', AI1WMKE_PLUGIN_NAME ), site_url() ) .
			sprintf( __( '<p>Date: %s</p>', AI1WMKE_PLUGIN_NAME ), date_i18n( 'r' ) );

		if ( $error ) {
			$notification .= sprintf( __( '<p>Error message: %s</p>', AI1WMKE_PLUGIN_NAME ), $error );
		}

		Ai1wm_Notification::error(
			sprintf( __( 'Scheduled event has failed (%s)', AI1WMKE_PLUGIN_NAME ), parse_url( site_url(), PHP_URL_HOST ) . parse_url( site_url(), PHP_URL_PATH ) ),
			$notification
		);
	}
	...
	public function logs() {
		...
		return array_map(
			function ( $log ) {
				...
				$log['status_locale'] = __( $log['status'], AI1WMKE_PLUGIN_NAME );
