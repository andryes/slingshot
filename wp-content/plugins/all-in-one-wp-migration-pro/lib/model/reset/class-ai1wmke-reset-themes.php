...
class Ai1wmke_Reset_Themes {
	...
	public static function execute( $params ) {
		...
		if ( ! isset( $params['ai1wm_reset_plugins'], $params['ai1wm_reset_themes'], $params['ai1wm_reset_media'], $params['ai1wm_reset_database'] ) ) {
			$message = __( 'Your theme has been successfully reset. All themes except the default WordPress theme have been removed, and the default theme is now active. This process has cleared all customizations and settings related to other themes. You can now start fresh with theme setup or install a new theme to customize your site\'s appearance.', AI1WMKE_PLUGIN_NAME );
			if ( defined( 'WP_CLI' ) ) {
	...
	protected static function create_default_theme() {
		// Create theme files
		Ai1wm_Directory::create( get_theme_root() . DIRECTORY_SEPARATOR . AI1WMKE_RESET_THEME_NAME );
		Ai1wm_File::create(
			get_theme_root() . DIRECTORY_SEPARATOR . AI1WMKE_RESET_THEME_STYLE_NAME,
			implode(
				PHP_EOL,
				array(
					'/*',
		...
		Ai1wm_File::create(
			get_theme_root() . DIRECTORY_SEPARATOR . AI1WMKE_RESET_THEME_INDEX_NAME,
			implode(
				PHP_EOL,
				array(
					'<!DOCTYPE html>',
		...
		return wp_get_theme( AI1WMKE_RESET_THEME_NAME );
	}
}
