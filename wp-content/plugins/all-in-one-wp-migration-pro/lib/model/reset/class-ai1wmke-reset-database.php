...
class Ai1wmke_Reset_Database {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['ai1wm_reset_plugins'], $params['ai1wm_reset_themes'], $params['ai1wm_reset_media'], $params['ai1wm_reset_database'] ) ) {
			...
			$message = __( "The reset of your site's database has been successfully completed. WordPress has been restored to its original state, similar to a new installation. Please remember, this action has removed all content, settings, and user data. Proceed to set up your site again or restore from a backup if necessary.", AI1WMKE_PLUGIN_NAME );
			if ( defined( 'WP_CLI' ) ) {
				WP_CLI::success( $message );
			} else {
				Ai1wm_Status::done( __( 'Reset Successful' ), $message );
	...
	protected function backup_options() {
		...
		$ai1wm_options = $wpdb->get_results( "SELECT `option_name`, `option_value` FROM `{$wpdb->options}` WHERE `option_name` LIKE 'ai1wm%%'" );
	...
	protected function save_backup() {
		if ( ! empty( $this->backup ) ) {
			$handle = ai1wm_open( ai1wmke_reset_db_backup_path(), 'w' );
	...
	protected function restore_data() {
		...
		// Read backup file
		$handle = ai1wm_open( ai1wmke_reset_db_backup_path(), 'r' );

		// Parse file
		$backup = ai1wm_read( $handle, filesize( ai1wmke_reset_db_backup_path() ) );
		...
		ai1wm_unlink( ai1wmke_reset_db_backup_path() );
	}
}
